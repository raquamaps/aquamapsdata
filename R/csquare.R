...
csquare_from_dd <- function(lat, lon, resolution = 0.0001) {
  ...
  decimal_split <- function(x, n_digits) {
    ...
    res <- as.integer(unlist(strsplit(split = "",
  ...
  quartet_beg <- function(lat, lon) {
    if (lat >= 0) return (ifelse(lon >= 0, 1, 7))
    return (ifelse(lon >= 0, 3, 5))
  }

  triplet_beg <- function(lat, lon) {
    if (lat <= 4 && lon <= 4) return (1)
    if (lat <= 4 && lon > 4) return (2)
    if (lat > 4 && lon <= 4) return (3)
    if (lat > 4 && lon > 4) return (4)
    warning("triplet_beg called with lat: ", lat, " lon: ", lon)
    return (NA)
  }

  pos1 <- purrr::map2_dbl(fractional_lat, fractional_lon, function(x, y) triplet_beg(x, y))
  pos2 <- fractional_lat
  pos3 <- fractional_lon
  ...
  quartet_head <- quartet_beg(as.integer(quartet_lat) * sign(lat), as.integer(quartet_lon) * sign(lon))
  ...
  triplet_one <- paste0(triplet_head, triplet_whole_lat, triplet_whole_lon)

  triplets <- paste(collapse = ":", c(quartet, triplet_one, fractional_triplets))
