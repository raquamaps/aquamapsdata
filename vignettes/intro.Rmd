...
Please remember to being your session with activating the connection to the downloaded database:
...
With a species identifier, likely locations of habitat can be displayed on a map, but native habitat data can also be retrieved in raster format.
...
## Maps that include several species

We can also display a map using several species identifiers, for example those associated with the  genus "Caranx", but should then provide an aggregation function such as "count", for example using:
...
The am_hspen() function provides the parameters for species' environmental envelopes; data on circumstances that the species are likely to tolerate. Information about this table is available in the help with fields explained in the `am_meta` dataset.
...
In the beginning of the vignette, the taxonomy name search functions were illustrated with some examples. These allow for getting a list of available mapped species based on a certain criteria, so that native habitat can be mapped for a set of species based on taxonomic criteria.

We can also use a function to list species occurring in an area; a location with cells identified by a single or multiple CsquareCode identifiers. Another function allows querying for species diversity or richness across a set of cells. Both these functions allows for specifying a minimum threshold for the probability that the cell is a suitable native species habitat.
